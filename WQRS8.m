...
%  usage: [QRS] = WQRS8(ECG,HEADER,PROGRESO,PAYLOAD)
%  param in
...
%/*------------------------------------------------------------------*/
function [QRS,MULTI]=WQRS8(ECG, HEADER,PROGRESO,PAYLOAD)
...
function [largo]=CurveLengthTrans(muestras, nsamples,nsig, fsamp)
	...
	ventana = 0.013;				% tiempo de la ventana en segundos >>0.13
	c = 1.25*200*200*1;				% Factor de escalamiento en segundos >>1.25
...
function [beat] = BloqueDecision(signal,Fs,gain)
	...
	DistMinima = 45;			% Consideracion = 45
	...
	while ((indice+ventana) < nsamp) 
		...
		if(max(signal(indice:indice+ventana)) > T1)
			...
			posMax = find(signal(indice:indice+ventana) > max(signal(indice:indice+ventana))*0.4);
