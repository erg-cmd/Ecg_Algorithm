...
%  usage: [QRS] = WQRS9(ECG,HEADER,PROGRESO,PAYLOAD)
%  param in
...
%/*------------------------------------------------------------------*/
function [QRS,MULTI]=WQRS9(ECG, HEADER,PROGRESO,PAYLOAD)
	...
	ECG_filtered=ECG_filtered(1:end-PhaseDelay,:);
...
function [largo]=CurveLengthTrans(muestras, nsamples,nsig, fsamp)
	...
	ventana = 0.130;				% tiempo de la ventana en segundos >>0.13
	c = 1.25*200*200;				% Factor de escalamiento en segundos >>1.25
...
function [beat] = BloqueDecision(signal,Fs,gain)
	EyeClosing = round(.35*Fs);	% periodo para evitar el sobredetección(ms)
	SegmQRS = round(.008*Fs);	% Segun wqrs 0.13s / segun yo 0.01
	...
	DistMinima = 35;			% Consideracion = 45
	...
	while ((indice+ventana) < nsamp) 
		...
		if(max(signal(indice:indice+ventana)) > T1)
			...
			posMax = find(signal(indice:indice+ventana) > max(signal(indice:indice+ventana))*0.5);
			%~ posMax = find(signal(indice:indice+ventana) > T1);
		...
		% No se detecto. Puede no haber o el Thres está alto!
			...
			tiempo = tiempo + ceil(ExpectedRR/800);
		...
		% nos corremos ExpectedRR/xxx
		indice = indice + ceil(ExpectedRR/800);
	end
	
	%/*-Tramo que falta desde indice hasta end------------------------*/
	% Ultimo QRS
	if(max(signal(end-ventana:end)) > T1)													
		% reubicamos indice
