...
%/*------------------------------------------------------------------*/
function [QRS,MULTI]=WQRS6(ECG, HEADER,PROGRESO,PAYLOAD)
	...
	LT = CurveLengthTrans(ECG_filtered,HEADER.nsamp,HEADER.nsig, HEADER.freq);
	minimo = min(LT);
...
function [largo]=CurveLengthTrans(muestras, nsamples,nsig, fsamp)
	...
	ventana = 0.025;				% tiempo de la ventana en segundos >>0.02
	c = 1.25*100000;				% Factor de escalamiento en segundos >>0.9
...
function [beat] = BloqueDecision(signal,Fs,gain)
	...
	SegmQRS = round(.13/Ts);	% Segun bibliografia consultada 130ms
